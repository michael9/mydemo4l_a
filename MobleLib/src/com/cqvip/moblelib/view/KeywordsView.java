...
public class KeywordsView extends FrameLayout implements OnGlobalLayoutListener {
	...
	public static final int MAX = Constant.DEFAULT_COUNT_SEARCHHISTORY;
	...
	private void disapper() {
		...
		for (int i = size - 1; i >= 0; i--) {
			...
			int[] xy = new int[] { layParams.leftMargin, layParams.topMargin,
					txt.getWidth() };
			// Log.e("---",
			// "disapper()-layParams.leftMargin="+layParams.leftMargin+
	...
	public boolean show() {
		if (width > 0 && height > 0 && vecKeywords != null
			...
			for (int i = 0; i < size; i++) {
				listX.add(random.nextInt(xItem * (i + 1)));
				// listX.add(i*xItem);
	...
	private AnimationSet getAnimationSet(int[] xy, int xCenter, int yCenter,
		...
		if (type == OUTSIDE_TO_LOCATION) {
			...
			TranslateAnimation translate = new TranslateAnimation(0, xCenter
					- xy[IDX_X], 0, yCenter - xy[IDX_Y]);
			animSet.addAnimation(translate);
		} else if (type == CENTER_TO_LOCATION) {
			animSet.addAnimation(animAlpha2Opaque);
			animSet.addAnimation(animScaleZero2Normal);
			TranslateAnimation tranlate = new TranslateAnimation(xCenter
					- xy[IDX_X], 0, yCenter - xy[IDX_Y], 0);
